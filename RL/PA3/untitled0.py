...
def SARSA(env, beta, Nepisodes, alpha):
    # Get the state dimension and action space
    n_actions = env.action_space.n  # 2 actions (keep or switch)
    
    # Initialize Q-table - using max queue length of 20 as specified
    max_queue_length = 20
    # Q-table structure: [queue1, queue2, green_road, time_since_switch, action]
    q_table = np.zeros((max_queue_length + 1, max_queue_length + 1, 2, 11, n_actions))
    
    # Hyperparameter for exploration strategy
    nu = 1e-5  # Small positive number to prevent division by zero
    ...
    # Helper function to truncate queue lengths to max_queue_length
    def truncate_state(state):
        # Truncate queue lengths but keep green_road and time_since_switch as is
        return [min(state[0], max_queue_length), 
                min(state[1], max_queue_length), 
                state[2], 
                min(state[3], 10)]  # Ensure time index is bounded at 10
    
    # Helper function for normalized Q-values for exploration
    ...
    # Helper function for action selection using epsilon-greedy with specified strategy
    def select_action(state, q_table, epsilon):
        q1, q2, green, time = state
        green_idx = 0 if green == 1 else 1  # Convert green road 1/2 to index 0/1
        time_idx = min(time, 10)  # Ensure time index doesn't exceed bounds
        
        q_values = q_table[q1, q2, green_idx, time_idx]
        
        if np.random.random() < epsilon:  # Explore
            # Normalize Q-values for the exploration strategy
            normalized_q = normalize_q_values(q_values)
            # Calculate probabilities according to the formula
    ...
    for episode in range(Nepisodes):
        ...
        state = env.reset()
        if isinstance(state, tuple):  # New Gymnasium API returns (obs, info)
            state = state[0]
        ...
        while not done:
            ...
            q1, q2, green, time = state
            next_q1, next_q2, next_green, next_time = next_state
            ...
            q_table[q1, q2, green_idx, time_idx, action] += alpha * (
                reward + beta * q_table[next_q1, next_q2, next_green_idx, next_time_idx, next_action] - 
                q_table[q1, q2, green_idx, time_idx, action]
            )
            
            # Update state and action for the next time step
            state = next_state
            action = next_action
    
    # Extract the policy from the Q-table (greedy policy)
    ...
    for q1 in range(max_queue_length + 1):
        for q2 in range(max_queue_length + 1):
            for g in range(2):  # green_road is 1 or 2, indexed as 0 or 1
                for t in range(11):  # time_since_switch from 0 to 10
                    policy[q1, q2, g, t] = np.argmax(q_table[q1, q2, g, t])
...
def ExpectedSARSA(env, beta, Nepisodes, alpha):
    ...
    # Helper function to truncate queue lengths
    def truncate_state(state):
        return [min(state[0], max_queue_length), 
                min(state[1], max_queue_length), 
                state[2], 
    ...
    # Helper function for normalized Q-values
    def normalize_q_values(q_values):
        min_q = np.min(q_values)
        max_q = np.max(q_values)
        denominator = max_q - min_q + nu
    ...
    # Helper function for action selection (epsilon-greedy)
    def select_action(state, q_table, epsilon):
        q1, q2, green, time = state
        green_idx = 0 if green == 1 else 1  # Convert green road 1/2 to index 0/1
        time_idx = min(time, 10)  # Ensure time index doesn't exceed bounds
    ...
    for episode in range(Nepisodes):
        ...
        while not done:
            ...
            next_q_values = q_table[next_q1, next_q2, next_green_idx, next_time_idx]
            normalized_q = normalize_q_values(next_q_values)
            exp_values = np.exp(normalized_q)
            action_probs = exp_values / np.sum(exp_values)
            
            expected_q = np.dot(action_probs, next_q_values)

            # Expected SARSA update
            q_table[q1, q2, green_idx, time_idx, action] += alpha * (
                reward + beta * expected_q - q_table[q1, q2, green_idx, time_idx, action]
            )

            # Select next action (following ε-greedy)
            action = select_action(next_state, q_table, epsilon)
            state = next_state
...
# Learn the optimal policies using different TD learning approaches
policy1 = SARSA(env, beta, Nepisodes, alpha)
policy2 = ExpectedSARSA(env, beta, Nepisodes, alpha)
# policy3 = ValueFunctionSARSA(env, beta, Nepisodes, alpha)  # Uncomment when implemented

# Save the policies
np.save('policy1.npy', policy1)
np.save('policy2.npy', policy2)
# np.save('policy3.npy', policy3)  # Uncomment when implemented
